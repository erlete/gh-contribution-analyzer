...
async def test_client_strips_inline_reasoning() -> None:
    def handler(request: httpx2.Request) -> httpx2.Response:
        return _completion("<think>counting commits...</think>\n14 PRs shipped.")

    transport = httpx2.MockTransport(handler)
    async with AIClient(_ai_config(), transport=transport) as client:
        result = await client.complete("sys", "user")
    assert result == "14 PRs shipped."


async def test_client_rejects_reasoning_only_response() -> None:
    def handler(request: httpx2.Request) -> httpx2.Response:
        return _completion("<think>still thinking</think>")

    transport = httpx2.MockTransport(handler)
    async with AIClient(_ai_config(), transport=transport) as client:
        with pytest.raises(AIError, match="only reasoning"):
...
async def test_insight_for_unconfigured_returns_fallback(
    ...
    result = await insights.insight_for(
        ...
        scope_key="org:acme",
        period_key="2026-W31",
        context={"totals": {"commits": 12, "active_people": 3, "active_repos": 2}},
    )
    assert result.ai is False
    assert "12" in result.text
    assert (await session.scalar(sa.select(sa.func.count(Insight.id)))) == 0
...
async def test_insight_for_generates_and_stores(
    ...
    assert result.text == "Alpha leads with 12 commits and 3 reviews."
...
async def test_insight_for_second_call_hits_cache(
    ...
    assert first.text == "Review load doubled to 18 this period."
    assert calls == 1
    second = await insights.insight_for(session, **kwargs)  # type: ignore[arg-type]
    assert second.text == first.text
    assert second.ai is True
    assert calls == 1


async def test_insight_for_failure_falls_back(
    session: AsyncSession, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("gca.ai.client._RETRY_SLEEP", 0.0)
    await SettingsStore(session).set_ai(_ai_config())
    ...
    result = await insights.insight_for(
        ...
        scope_key="org:acme",
        period_key="2026-W31",
        context={"totals": {"commits": 7, "active_people": 2, "active_repos": 1}},
    )
    assert result.ai is False
...
async def test_instructions_are_part_of_cache_key(
    session: AsyncSession, monkeypatch: pytest.MonkeyPatch
) -> None:
    await SettingsStore(session).set_ai(_ai_config())
    calls = 0
    ...
    kwargs: dict[str, object] = {
        ...
        "scope_key": "all",
        "period_key": "30d",
        "context": {"totals": {"commits": 5}},
        "area": "dashboard",
    }
    first = await insights.insight_for(session, **kwargs)  # type: ignore[arg-type]
    assert calls == 1
    await SettingsStore(session).set_ai_instructions({"dashboard": "focus on reviews"})
    second = await insights.insight_for(session, **kwargs)  # type: ignore[arg-type]
    assert calls == 2
    assert second.text != first.text
